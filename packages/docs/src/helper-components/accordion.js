...
export function AccordionItem({ toggle, children, beforeText, afterText }) {
  ...
  return (
    <div role="button" onClick={toggleItem(toggle)} className={style.item}>
      ...
      <div className="flex justify-center py-4">
        {selected === toggle ? (
          <AngleUpIcon afterText={afterText} />
        ) : (
          <AngleDownIcon beforeText={beforeText} />
        )}
      </div>
    </div>
  );
...
const AngleUpIcon = (props) => (
  <button className="flex flex-wrap justify-center items-center gap-2 bg-transparent border-0 cursor-pointer">
    <div className="font-monts font-bold text-lg">{props.afterText}</div>
    <svg
      fill="white"
      strokeWidth="1"
      viewBox="0 0 320 512"
...
const AngleDownIcon = (props) => (
  <button className="flex flex-wrap justify-center items-center gap-2 bg-transparent border-0 cursor-pointer">
    <div className="font-monts font-bold text-lg">{props.beforeText}</div>
    <svg
      stroke="currentColor"
      fill="white"
      strokeWidth="0"
