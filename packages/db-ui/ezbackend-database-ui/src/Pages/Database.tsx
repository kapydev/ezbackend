...
function Database() {
  ...
  useEffect(() => {
    fetch(`${URL}/docs/json`)
      ...
      .then((data) => {
        ...
        const newData: ISchema[] = data.map((item: IJsonSchema) => {
          return { schemaName: item.title, properties: item.properties };
        });
        ...
        const modelnames: string[] = fullschemas.map((schema) => {
        ...
        const initialSelectedItem = modelnames[0];
      ...
      if (routeName === schema.schemaName) {
        setColumnNames(
          Object.keys(schema.properties).map((property) => {
            const [charLenThreshHold, maxCellWidth, widthMultiplier] = [
              5, 150, 45,
            ];
            return {
              field: property,
  ...
  function handleGetRowData(routeName: string) {
    const fetchPromise = fetch(`${URL}/${routeName}/`)
      .then(function (response) {
        if (!response.ok) {
          throw Error(response.statusText);
        }
  ...
  function handlePatchSelectedCell(patchdata: GridCellEditCommitParams) {
    // @ts-ignore
    const id: number = patchdata.id;
    const body: any = {};
    ...
    const fetchPromise = fetch(`${URL}/${selectedItem}/${id}`, requestOptions)
      .then(function (response) {
        if (!response.ok) {
          throw Error(response.statusText);
        }
  ...
  function getCreateSchema() {
    const createschemas: IJsonSchema[] = [...createSchemas];
    const c = createschemas?.filter((createschema) => {
      ...
      <Box bgcolor="#EEEEEE" minHeight="100vh" padding={4}>
        <Grid container direction="row" justifyContent="center" spacing={5}>
                  ...
                  <Grid item xs>
                    <Grid container justifyContent="flex-end">
                      <Grid item>
                        <IconButton
                          color="primary"
                          onClick={handleOpenPostRequestDialog}
                        >
                          <AddIcon />
                        </IconButton>
                      </Grid>
          ...
          <Typography variant="body1" align="center">
            <Box fontFamily="monospace">Powered by EzBackend</Box>
          </Typography>
        </Grid>
        <PostRequestDialog
        ...
        />
        <Toaster position="bottom-left" toastOptions={{ duration: 800 }} />
      </Box>
    </Fade>
  );
}
