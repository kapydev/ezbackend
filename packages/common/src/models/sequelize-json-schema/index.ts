...
function _includeAttribute(opts, name) {
  const include =
    (!opts.exclude || !opts.exclude.includes(name)) &&
    (!opts.attributes ||
      opts.attributes.length <= 0 ||
      opts.attributes.includes(name));
  return include;
}
 ...
 */
export function getAttributeSchema(att) {
  let schema;
  ...
  switch (attType) {
    ...
    case "STRING": {
      ...
      let length =
        att.type._length || (att.type.options && att.type.options.length);
 ...
 * `attributes`)
 */
export function getModelSchema(model, options = {}) {
  ...
  if (options.alwaysRequired) {
    throw Error(
      "`alwaysRequired` option is no longer supported (Add required properties `schema.required[]` in the returned schema"
    );
  }
...
export function getSequelizeSchema(seq, options = {}) {
